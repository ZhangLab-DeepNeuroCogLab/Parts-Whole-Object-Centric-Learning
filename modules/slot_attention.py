...
from modules.utils import gru_cell, linear
...
class SlotAttention(nn.Module):
    ...
    def forward(self, inputs, slots_init):
        ...
        for _ in range(self.num_iterations):
            ...
            attn = F.softmax(attn_logits.transpose(1, 2).reshape(B, N_kv, self.num_heads * N_q), dim=-1).view(B, N_kv, self.num_heads, N_q).transpose(1, 2)  # Shape: [batch_size, num_heads, num_inputs, num_slots].
            attn_vis = attn.sum(1)  # Shape: [batch_size, num_inputs, num_slots].

            # Weighted mean.
            attn = attn + self.epsilon
            attn = attn / torch.sum(attn, dim=-2, keepdim=True) # [batch_size, num_heads, num_inputs, num_slots].
...
class SlotAttentionEncoder(nn.Module):
    ...
    def forward(self, x, slots=None):
        ...
        if slots is None:
            ...
            slots = self.slot_mu + torch.exp(self.slot_log_sigma) * z
